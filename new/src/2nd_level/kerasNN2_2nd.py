...
print("Loading data...")
X, labels = load_data(path+'train_2nd.csv', train=True)
#X=np.log(X+1)
#X=np.sqrt(X+(3/8))
...
X_test, ids = load_data(path+'test_2nd.csv', train=False)
...
for tr, te in skf:
	...
	for j in range(n_bag):
		...
		model = Sequential()
		model.add(Dense(512, input_shape=(dims,))) # 768 
		model.add(PReLU())
...
make_submission(pred, ids, encoder, fname=path+'kerasNN2_2nd.csv')
print(log_loss(labels,submissionTr.values,eps=1e-15, normalize=True))
submissionTr.to_csv(path+"kerasNN2_2nd_retrain.csv",index_label='id')

# nfold 5, bagging 10: 0.41880789 + 0.0071999
# nfold 8, bagging 10: 0.4222390 + 0.0072095
